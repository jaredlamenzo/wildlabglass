...
function store_credentials($user_id, $credentials) {
  $db = init_db();
  $user_id = mysqli::escapeString(strip_tags($user_id));
...
function list_credentials() {
  ...
  $result = array();
  while ($singleResult = $query->fetchArray(MYSQLI_ASSOC)){
...
function init_db() {
  global $mysqli_db;
   ...
   $db = new mysqli($mysqli_db);
  $test_query = "select count(*) from mysqli_master where name = 'credentials'";
